...
class ZCoreConst {
  ...
  public parse(json: any, target: any) {
    this.recursiveParse(target, json);
  }
...
export abstract class ZCoreBase {
  ...
  /**
   * 序列化当前对象 !!!请优先使用Create方法创建对象,此方法用于特殊情况需要优先赋值的时候。
   * sample:
   * if (propertyName === 'bill_detail') {
      switch (this.bill_type) {
        case 'booking':
          return BillBookingDetailEntity;
        case 'fee':
          return BillFeeDetailEntity;
        case 'service':
          return BillServiceDetailEntity;
        case 'refund': // 目前只有服务的退款
          return BillServiceDetailEntity;
        case 'arrearage':
          return BillArrearageDetailEntity;
        case 'short_message':
          return BillShortMessageDetailEntity;
      }
    }
   使用了数据中的bill_type属性做判断
   * @param json
   */
  public toInstance(json: any) {
    ZCORE.parse(json, this);
  }
