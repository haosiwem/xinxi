...
export class DurationFormatHMSPipe implements PipeTransform {
  transform(duration, ignoreSeconds: boolean = false): string {
    ...
    if (ignoreSeconds) {
      ...
      if (s != 0) {
        second = second - s + 60; // 当秒数不为0时进位(进一分钟)
      }
    }

    let day = Math.floor(Number(second / (3600 * 24)));
    let hour = Math.floor(Number(second % (3600 * 24) / 3600));
    let m = Math.floor(Number(second % 3600 / 60));
    let s = Math.floor(Number(second % 3600 % 60));
    if (day == 0) {
      if (hour != 0) {
        if (m != 0) {
          if (s != 0) {
